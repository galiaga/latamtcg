...
async function handle(req: NextRequest) {
  try {
    ...
    console.log('[cron] Stage+Update request details:', {
    ...
    console.log('[cron] Starting Vercel Stage+Update...')
    
    const { VercelStageUpdatePipeline } = await import('@/scripts/vercel-ingest-stage-update')
    const pipeline = new VercelStageUpdatePipeline()
    
    // Use URL mode to download CSV directly from Scryfall
    const result = await pipeline.ingest({ url: 'https://api.scryfall.com/bulk-data/default-cards' })
    
    console.log('[cron] Vercel Stage+Update completed:', result)
    return NextResponse.json(result)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- logging path, not critical to type precisely
  } catch (err: any) {
    console.error('[cron] Stage+Update job failed', err)
