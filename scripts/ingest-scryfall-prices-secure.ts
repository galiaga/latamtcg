...
export class SecurePriceIngestionPipeline {
  private client: Client
  private auditRun: AuditRun

  constructor() {
    // Use Session Pooler URL with proper SSL configuration
    let connectionString = process.env.DATABASE_URL // Use Session Pooler URL
  ...
  private getSSLConfig() {
    ...
    // Fallback: Log warning and use insecure mode for development
    console.warn('[ssl] ⚠️  WARNING: No Supabase CA certificate found!')
    console.warn('[ssl] ⚠️  Using insecure SSL mode. This is NOT recommended for production.')
    console.warn('[ssl] ⚠️  To fix: Download Supabase CA certificate and set SUPABASE_CA_PEM environment variable')
  ...
  async ingest(options: IngestOptions): Promise<IngestResult> {
    ...
    try {
      ...
      // Log start of run (but don't log until we have priceDay)
      console.log(`[ingest] Starting secure ingestion run...`)
      ...
      console.log(`[ingest] Started secure ingestion run #${runId}`)
      ...
      console.log(`[ingest] 🎉 Secure ingestion completed!`)
      ...
      console.error(`[ingest] ❌ Secure ingestion failed after ${totalMs}ms:`, error.message)
      throw error
    } finally {
...
SSL Configuration:
  ...
  Or place supabase-ca.pem file in project root
        `)
        process.exit(0)
    }
  }
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      console.log('✅ Secure price ingestion completed successfully')
    }
